...
function courseTint(index: number) {
  const tints = [
    "bg-fuchsia-50 border-fuchsia-100 dark:bg-fuchsia-950/40 dark:border-fuchsia-800/30",
    "bg-teal-50 border-teal-100 dark:bg-teal-950/40 dark:border-teal-800/30",
    "bg-violet-50 border-violet-100 dark:bg-violet-950/40 dark:border-violet-800/30",
    "bg-amber-50 border-amber-100 dark:bg-amber-950/40 dark:border-amber-800/30",
    "bg-sky-50 border-sky-100 dark:bg-sky-950/40 dark:border-sky-800/30",
    "bg-rose-50 border-rose-100 dark:bg-rose-950/40 dark:border-rose-800/30",
    "bg-lime-50 border-lime-100 dark:bg-lime-950/40 dark:border-lime-800/30",
    "bg-indigo-50 border-indigo-100 dark:bg-indigo-950/40 dark:border-indigo-800/30",
    "bg-orange-50 border-orange-100 dark:bg-orange-950/40 dark:border-orange-800/30",
    "bg-cyan-50 border-cyan-100 dark:bg-cyan-950/40 dark:border-cyan-800/30",
  ];
      ...
      <StaggerGroup className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {popularCourses.map((course, index) => (
          <StaggerItem key={course.id} className="h-full">
            ...
            >
              <span className="block text-base font-bold text-neutral-950 dark:text-white">
                {course.name}
              </span>
              <span className="mt-1 block text-xs text-neutral-500 dark:text-neutral-400">{course.tag}</span>
              <span className="mt-3 block text-xs font-medium text-neutral-600 dark:text-neutral-300">
...
function CollegeCard({ college }: { college: College }) {
  return (
    <Link
      ...
      className="flex h-full flex-col rounded-2xl border border-black/5 bg-white dark:bg-neutral-900/90 dark:border-white/10 p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
    >
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-base font-semibold leading-6 text-neutral-950 dark:text-white">
          {college.name}
        </h3>
        {college.verified && (
      ...
      <div className="mt-4 flex items-center gap-1 text-sm text-neutral-700 dark:text-neutral-300">
        <StarIcon className="h-4 w-4 text-amber-400" />
        <span className="font-semibold">{college.rating}</span>
      </div>
      <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-black/5 dark:border-white/10 pt-4">
        <div>
          <dt className="text-xs text-neutral-500 dark:text-neutral-400">Annual Fees</dt>
          <dd className="mt-0.5 text-sm font-semibold text-neutral-900 dark:text-white">
            {college.fees}
          </dd>
        </div>
        <div>
          <dt className="text-xs text-neutral-500 dark:text-neutral-400">Placement</dt>
...
export function FeaturedColleges() {
  return (
    <section className="bg-neutral-50 dark:bg-neutral-900/40 py-14">
        ...
        {scholarships.map((s) => (
          <StaggerItem key={s.id} className="h-full">
            ...
            >
              ...
              <h3 className="mt-4 text-base font-semibold leading-6 text-neutral-950 dark:text-white">
                {s.name}
              </h3>
              <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">{s.provider}</p>
              <div className="mt-4 flex items-end justify-between gap-2 border-t border-black/5 dark:border-white/10 pt-4">
                <div>
                  <p className="text-xs text-neutral-500 dark:text-neutral-400">Scholarship amount</p>
                  <p className="text-sm font-bold text-emerald-700 dark:text-emerald-400">{s.amount}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-neutral-500 dark:text-neutral-400">Deadline</p>
                  <p className="text-sm font-medium text-neutral-800 dark:text-neutral-200">{s.deadline}</p>
                </div>
              </div>
            </Link>
          </StaggerItem>
          ...
          {upcomingExams.map((exam) => (
            <StaggerItem key={exam.id} className="h-full">
              <Link
                ...
                className="block h-full rounded-2xl border border-black/5 bg-white dark:bg-neutral-900/90 dark:border-white/10 p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
              >
                <span className="inline-flex rounded-full bg-violet-50 text-violet-700 dark:bg-violet-950/60 dark:text-violet-300 px-3 py-1 text-xs font-semibold">
                  {exam.status}
                </span>
                <h3 className="mt-4 text-base font-semibold text-neutral-950 dark:text-white">
                  {exam.name}
                </h3>
                <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">Exam date: {exam.date}</p>
              </Link>
            </StaggerItem>
          ))}
        </StaggerGroup>
          ...
          <StaggerItem key={test.id} className="h-full">
            ...
            >
              <div className="flex items-center justify-between">
                <ClipboardIcon className="h-6 w-6 text-indigo-600 dark:text-indigo-400" />
                <span
